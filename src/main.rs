...
fn main() {
    ...
    let mut organisation_hashmap:HashMap<&str, String> = HashMap::new();
    let mut list_of_staff  = Vec::new();
    let mut list_of_department  = Vec::new();
    loop {
        ...
        let user_input_vector:Vec<_> = user_input.split_whitespace().collect();
        ...
        match user_input_length {
              ...
              let result = user_input_sorter::for_three_length_input(&user_input);
               match result {
                ...
                //   let user_input_vector:Vec<&str> = user_input.split_whitespace().collect();
                  ...
                  let does_department_match = user_input_sorter::check_if_department_match(department, &list_of_department);

                  match does_department_match {
                      true => {
                       let department_staffs = organisation_hashmap.get(department).unwrap();
            ...
            4 => {
                let result = user_input_sorter::for_four_length_input(&user_input);
                     match result {
                    ...
                    1 => {
                        ...
                        list_of_staff.push(person_name);
                        list_of_department.push(department_name);
                        let new_staff =  
                        organisation_hashmap.entry(department_name)
                        .or_insert(String::from(""));
                    new_staff.push_str(" ");
                    new_staff.push_str(person_name);
                    
                    println!("{} have to added to {} department", person_name, department_name)
                },
                ...
                2 => {
                    ...
                    let does_department_match = 
                               user_input_sorter::check_if_department_match(department_name, &list_of_department);

                    match does_department_match {
                        true => {
                          let list_of_staffs_in_the_department = 
                                                organisation_hashmap
                                                .get(department_name).unwrap();
                          ...
                          if let Some(staff) = list_of_staffs_vec.iter()
                                                    .position(|x| *x == person_name){
                            list_of_staffs_vec.remove(staff);
                            let edited_list_of_staffs = list_of_staffs_vec.join(" ");
                            organisation_hashmap
                            .insert(department_name, String::from(edited_list_of_staffs));

                            if let Some(staff) = list_of_staff.iter()
                                                    .position(|x| *x == person_name){
